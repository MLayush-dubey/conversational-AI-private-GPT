import uuid  #inbuilt py lib to generate unique ID
...
conversations.create_index([("last_interacted", DESCENDING)])  #ordering the chat convo history based on the most recent interaction
...
def create_new_conversation_id() -> str:
    return str(uuid.uuid4())  #we create a new id when user clicks on new conversation
...
def create_new_conversation(title: Optional[str] = None, role: Optional[str] = None, content: Optional[str] = None) -> str:
    ...
    if role and content:
        doc["messages"].append({"role": role, "content": content})
